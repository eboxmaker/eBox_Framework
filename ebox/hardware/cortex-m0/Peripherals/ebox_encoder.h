...
class Encoder:T_base
{
    public:
        Encoder(TIM_TypeDef *timer,Gpio *a,Gpio *b):T_base(timer){
            //this->TIMx = timer;
            this->a = a;
            this->b = b;
        };
        Encoder(TIM_TypeDef *timer,Gpio *a):T_base(timer){
            this->a = a;
        };
        bool begin(uint32_t pulse_per_circle,uint8_t mode = LL_TIM_ENCODERMODE_X4_TI12);
    ...
    private:
        ...
        void set_encoder(uint8_t mode = LL_TIM_ENCODERMODE_X4_TI12);
