...
enum spif_bd_error {
    ...
    SPIF_BD_ERROR_DEVICE_ERROR          = ebox::BD_ERROR_DEVICE_ERROR, /*!< device specific error -4001 */
 ...
 *  @endcode
 */
class SPIFBlockDevice : public ebox::BlockDevice {
public:
     ...
     *                  SPIF_BD_ERROR_DEVICE_ERROR - device driver transaction failed
     */
    virtual int read(void *buffer, ebox::bd_addr_t addr, ebox::bd_size_t size);
     ...
     */
    virtual int program(const void *buffer, ebox::bd_addr_t addr, ebox::bd_size_t size);
     ...
     */
    virtual int erase(ebox::bd_addr_t addr, ebox::bd_size_t size);

    /** Get the size of a readable block
     *
     *  @return         Size of a readable block in bytes
     */
    virtual ebox::bd_size_t get_read_size() const;
     ...
     */
    virtual ebox::bd_size_t get_program_size() const;
     ...
     */
    virtual ebox::bd_size_t get_erase_size() const;
     ...
     *  @note Must be a multiple of the program size
     */
    virtual ebox::bd_size_t get_erase_size(ebox::bd_addr_t addr) const;
     ...
     */
    virtual ebox::bd_size_t size() const;
...
private:
    ...
    int _utils_find_addr_region(ebox::bd_size_t offset) const;
    ...
    spif_bd_error _spi_send_program_command(int prog_inst, const void *buffer, ebox::bd_addr_t addr, ebox::bd_size_t size);

    // Send Read command to Driver
    //spif_bd_error _spi_send_read_command(uint8_t read_inst, void *buffer, bd_addr_t addr, bd_size_t size);
    spif_bd_error _spi_send_read_command(int read_inst, uint8_t *buffer, ebox::bd_addr_t addr, ebox::bd_size_t size);

    // Send Erase Instruction using command_transfer command to Driver
    spif_bd_error _spi_send_erase_command(int erase_inst, ebox::bd_addr_t addr, ebox::bd_size_t size);

    // Send Generic command_transfer command to Driver
    spif_bd_error _spi_send_general_command(int instruction, ebox::bd_addr_t addr, char *tx_buffer,
