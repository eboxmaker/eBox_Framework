...
//    virtual int8_t      release_i2c_right(void) = 0;
	...
	virtual uint8_t	writeBuf(uint8_t slaveAddr,uint16_t regAddr,uint8_t *data, uint16_t num_to_write,uint16_t tOut) = 0;
	
	virtual uint8_t	readBuf(uint8_t slaveAddr,uint8_t *data, uint16_t num_to_read,uint16_t tOut) = 0;
	virtual uint8_t	readBuf(uint8_t slaveAddr, uint16_t regAddr, uint8_t *data,uint16_t num_to_read,uint16_t tOut) = 0;
	// 等待设备响应
