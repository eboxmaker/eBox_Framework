...
extern uint8 _socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag); // Opens a socket(TCP or UDP or IP_RAW mode)
extern void _close(SOCKET s); // Close socket
extern uint8 _connect(SOCKET s, uint8 * addr, uint16 port); // Establish TCP connection (Active connection)
