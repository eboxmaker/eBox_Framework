...
_TM1638::_TM1638(GPIO *p_data_pin, GPIO *p_sck_pin, GPIO *p_rck_pin)
{
    data_pin 	= p_data_pin;
    sck_pin 	= p_sck_pin;
    rck_pin 	= p_rck_pin;
...
void _TM1638::delay(uint16_t delayvalue)
{
    uint16_t i = 0;
    for(i = 0; i < delayvalue; i++);
}

void _TM1638::write(uint8_t DATA)
{
    uint8_t i;
    for(i = 0; i < 8; i++)
    {
...
void _TM1638::writeCmd(uint8_t Cmd)
{
    rck_pin->write(LOW);
    delay(10);
...
void _TM1638::LedDisplay(uint8_t ddata0, uint8_t ddata1, uint8_t ddata2, uint8_t ddata3,
