...
#if EBOX_DEBUG
// 是否打印调试信息, 1打印,0不打印
#define EBOX_DEBUG_RC522_ENABLE       true
#define EBOX_DEBUG_RC522_ENABLE_ERR   true
#endif


#if EBOX_DEBUG_RC522_ENABLE
#define rc522Debug(...)  ebox_printf("[rc522 DBG]:%d: ",__LINE__),ebox_printf(__VA_ARGS__ )
#else
#define rc522Debug(...)
#endif

#if EBOX_DEBUG_RC522_ENABLE_ERR
#define rc522DebugErr(fmt, ...)  ebox_printf("[rc522 err]:%d: " fmt "\n", __LINE__, __VA_ARGS__)
#else
#define rc522DebugErr(fmt, ...)
#endif
 ...
 ******************************************************************************/
RFID::RFID(Spi* spi, Gpio * chipSelectPin, Gpio * NRSTPD)
{
...
void RFID::init()
{
    ...
    _chipSelectPin->mode(OUTPUT_PP_PD);     // 设置管脚_chipSelectPin为输出并连接到模块使能口
    _chipSelectPin->reset();


    _NRSTPD->mode(OUTPUT_PP_PU);// 设置管脚NRSTPD为输出，非重置或掉电
    _NRSTPD->set();
    
    cfg.dev_num = _chipSelectPin->id;
    cfg.mode = Spi::MODE0;
    ...
    _NRSTPD->set();
    reset();
    int value =  readMFRC522(VersionReg);
    rc522Debug("version:%d",value);
